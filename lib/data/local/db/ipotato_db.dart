...
class IPotatoDatabase extends _$IPotatoDatabase {
  ...
  Future insertTask(TaskListCompanion task) => into(taskList).insert(task);

  Future updateTask(SingleTask task) => update(taskList).replace(task);
  Future deleteTask(SingleTask task) => delete(taskList).delete(task);
}
